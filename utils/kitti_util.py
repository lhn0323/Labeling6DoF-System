...
class Object3d(object):
    ...
    def __init__(self, label_file_line):
        ...
        self.t = (data[11], data[12], data[13])  # location (x,y,z) in rectified camera 0.
...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, from_video=False):
        ...
        self.P = calibs["P2"]
        self.P = np.reshape(self.P, [3, 4]) # 投影举证
        # Rigid transform from Velodyne coord to reference camera coord
        self.V2C = calibs["Tr_velo_to_cam"] # Velodyne坐标系到相机坐标系的变换矩阵
        self.V2C = np.reshape(self.V2C, [3, 4])
        self.C2V = inverse_rigid_trans(self.V2C)
        # Rotation from reference camera coord to rect camera coord
        self.R0 = calibs["R0_rect"] # 旋转矩阵对点云矫正,是参考相机坐标系和矫正后的相机坐标系之间的旋转关系
        self.R0 = np.reshape(self.R0, [3, 3])

        # Camera intrinsics and extrinsics
        self.c_u = self.P[0, 2]# 主点横坐标
        self.c_v = self.P[1, 2] # 主点纵坐标
        self.f_u = self.P[0, 0] # 横向焦距
        self.f_v = self.P[1, 1] # 纵向焦距
        self.b_x = self.P[0, 3] / (-self.f_u)  # relative 水平基线长度fu.bx =self.P[0, 3]
        self.b_y = self.P[1, 3] / (-self.f_v) # 垂直基线长度
    ...
    # 将3D点从相机0变换到相机2
    def project_rect0_to_rect2(self, pts_3d_rect, fov=88.8, w=1920, h=1080):
        """ Input: nx3 points in rect camera0 coord.
            Output: nx3 points in rectified camera2 coord.
        """
        pts_3d_rect = self.cart2hom(pts_3d_rect)

        # from (x, y z) in rectified camera 0 to (u, v, z) in rectified camera 2
        pts_3d = np.dot(pts_3d_rect, np.transpose(self.P))  # nx3
        ...
        pts_3d_rect2 = np.transpose(np.matmul(np.linalg.inv(camera2), np.transpose(pts_3d)))
...
def inverse_rigid_trans(Tr):
    ...
    inv_Tr[0:3, 0:3] = np.transpose(Tr[0:3, 0:3]) # 计算新的R=R的转置矩阵
    inv_Tr[0:3, 3] = np.dot(-np.transpose(Tr[0:3, 0:3]), Tr[0:3, 3]) # 计算新的T= - R的转置矩阵.T
    return inv_Tr
...
# )
MATRIX_Mt = np.array([
    [-2.447634620263e-03, -9.997474919061e-01, -2.233826994621e-02, 1.618821500214e-01],
    [-2.165155869370e-01, 2.233826994621e-02, -9.760235784144e-01, 7.073099023593e+00],
    [9.762760865083e-01, 2.447634620263e-03, -2.165155869370e-01, 1.569056547839e+00],
    [0.0, 0.0, 0.0, 1.0]
])
MATRIX_Kt = np.array([
    [2186.345947266, 0.0, 966.2516479492],
    [0.0, 2333.918457031, 582.0673217773],
    [0.0, 0.0, 1.0]
])
