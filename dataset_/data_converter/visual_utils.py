...
def test_draw_3d_box_on_image(image, gt_boxes, P2, denorm, c=(0, 255, 0)):
    
    for line in gt_boxes:
      dim = np.array(line[:3]).astype(float)
      location = np.array(line[3:6]).astype(float)
      rotation_y = float(line[7])
      box_3d = compute_box_3d_camera(dim, location, rotation_y, denorm)# 计算3D边界框在相机坐标系中的8个顶点坐标
      box_2d = project_to_image(box_3d, P2)# 将3D点投影到2D图像平面上，以获得它们在图像上的2D坐标。
